...
export default function RegistryPage() {
  ...
  // 5. THE RESTORE (UNREVOKE) ACTION
  const handleRestore = async (citizen) => {
    const confirmRestore = window.confirm(`Restore License for ${citizen.name} (Index #${citizen.index})?`);
    if (!confirmRestore) return;

    // Safety check: Ensure the database still has their real hash!
    if (!citizen.leafHash || citizen.leafHash === "0") {
        alert("❌ Error: Cannot restore. The original Leaf Hash was deleted from the database.");
        return;
    }
    ...
    try {
      ...
      // --- 1. RECALCULATE TREE WITH THE REAL HASH RESTORED ---
      const indexedCitizens = citizens.filter(c => c.index !== null).sort((a, b) => a.index - b.index);
      
      // We map through everyone. If they are revoked, keep them as "0". 
      // BUT if it's the citizen we are restoring, put their real leafHash back in!
      let updatedLeaves = indexedCitizens.map(c => {
          if (c.index === citizen.index) return citizen.leafHash; // Put the real car back in the parking spot
          return c.status === "Revoked" ? "0" : c.leafHash;       // Leave others alone
      });
      ...
      // --- 2. UPDATE NATIONAL CHAIN (HARDHAT) ---
      const localProvider = new ethers.JsonRpcProvider("http://127.0.0.1:8545");
      const localWallet = new ethers.Wallet(import.meta.env.VITE_ADMIN_PRIVATE_KEY, localProvider);
      const localContract = new ethers.Contract(LOCAL_REGISTRY_ADDRESS, REGISTRY_ABI, localWallet);

      // --- 3. UPDATE PUBLIC CHAIN (SEPOLIA) ---
      if (!window.ethereum) throw new Error("MetaMask not found!");
      const browserProvider = new ethers.BrowserProvider(window.ethereum);
      const signer = await browserProvider.getSigner();
      const publicAnchor = new ethers.Contract(PUBLIC_ANCHOR_ADDRESS, ANCHOR_ABI, signer);

      // Execute Blockchain Txs
      // Assuming your Hardhat contract uses 'Active' to update/re-insert state
      const txA = await localContract.Active(citizen.leafHash, realNewRoot); 
      await txA.wait();
      ...
      // --- 4. UPDATE MONGODB ---
      // You will need a backend route for this (e.g., /api/restore) that changes status back to "Active"
      await fetch(`${BACKEND_URL}/api/restore`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ driverIndex: citizen.index, newRoot: realNewRoot })
      });
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="bg-white dark:bg-[#151A22] rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
        {loading ? (
          <div className="p-8 text-center animate-pulse font-bold text-blue-500">
            Processing Dual-Chain Sync... Please check MetaMask.
          </div>
        ) : (
                      ...
                      {isPending && (
                        <div className="flex gap-2">
                          <input 
                            ...
                            className="w-full p-2 text-xs font-mono bg-gray-50 dark:bg-black border dark:border-gray-700 rounded outline-none focus:border-blue-500"
                          />
                          <button onClick={() => handleActivate(citizen.subject.licenseID)} className="px-3 py-1 bg-emerald-600 text-white text-xs font-bold rounded hover:bg-emerald-500 transition-colors">Activate</button>
                        </div>
                      )}
                      {isActive && (
                        <button onClick={() => handleRevoke(citizen.index)} className="text-red-600 font-semibold text-sm hover:text-red-500 transition-colors">
                          Revoke License
                        </button>
                      )}
                      {isRevoked && (
                        <button onClick={() => handleRestore(citizen)} className="text-blue-600 font-semibold text-sm hover:text-blue-500 transition-colors">
                          Restore License
                        </button>
                      )}
                    </td>
